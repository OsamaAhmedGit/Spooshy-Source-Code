...
namespace Rendering
{
	RTTI_DEFINITIONS(ExitDoor)

		const float ExitDoor::DefaultMovementRate = 10.0f;

	ExitDoor::ExitDoor(Game& game, Camera& camera, const std::string modelFilename)
	...
	void ExitDoor::Initialize()
	{
		...
		std::wstring textureName = L"Content\\Textures\\door2.jpg";
	...
	void ExitDoor::Update(const GameTime& gameTime)
	{
		mAngle += XM_PI * static_cast<float>(gameTime.ElapsedGameTime() / 2);
	}


	void ExitDoor::Draw(const GameTime& gameTime)
	{
	...
	void ExitDoor::SetPosition(const float rotateX, const float rotateY, const float rotateZ, const float scaleFactor, const float translateX, const float translateY, const float translateZ)
	{
		XMMATRIX worldMatrix = XMLoadFloat4x4(&mWorldMatrix);
	...
	void ExitDoor::CreateVertexBuffer(ID3D11Device* device, const Mesh& mesh, ID3D11Buffer** vertexBuffer) const
	{
