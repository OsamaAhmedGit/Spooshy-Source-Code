...
namespace Rendering
{
	RTTI_DEFINITIONS(Key)

		const float Key::DefaultMovementRate = 10.0f;

	Key::Key(Game& game, Camera& camera, const std::string modelFilename)
	...
	void Key::Initialize()
	{
		...
		std::wstring textureName = L"Content\\Textures\\key.bmp";
	...
	void Key::Update(const GameTime& gameTime)
	{
		mAngle += XM_PI * static_cast<float>(gameTime.ElapsedGameTime() / 2);
	}


	void Key::Draw(const GameTime& gameTime)
	{
	...
	void Key::SetPosition(const float rotateX, const float rotateY, const float rotateZ, const float scaleFactor, const float translateX, const float translateY, const float translateZ)
	{
		XMMATRIX worldMatrix = XMLoadFloat4x4(&mWorldMatrix);
	...
	void Key::CreateVertexBuffer(ID3D11Device* device, const Mesh& mesh, ID3D11Buffer** vertexBuffer) const
	{
