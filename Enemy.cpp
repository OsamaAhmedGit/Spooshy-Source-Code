...
namespace Rendering
{
	RTTI_DEFINITIONS(Enemy)
		...
		const float Enemy::DefaultMovementRate = 10.0f;

	Enemy::Enemy(Game& game, Camera& camera, const std::string modelFilename)
		...
		mInputLayout(nullptr), mWorldMatrix(MatrixHelper::Identity), mVertexBuffer(nullptr), mIndexBuffer(nullptr), mIndexCount(0), modelFile(modelFilename), mPosition(0, 0, 0), mAngle(0.0)
	{
		pos = Vector3Helper::Zero;
	}
	...
	void Enemy::Initialize()
	{
		...
		std::wstring textureName = L"Content\\Textures\\armour.jpg";
	...
	{
		enemyCollisionBox.vecMax = { pos.x + (float)3, pos.y + (float)3, pos.z + (float)3 };
		enemyCollisionBox.vecMin = { pos.x - (float)3, pos.y, pos.z - (float)3 };
	...
	void Enemy::SetPosition(const float rotateX, const float rotateY, const float rotateZ, const float scaleFactor, const float translateX, const float translateY, const float translateZ)
	{
		XMMATRIX worldMatrix = XMLoadFloat4x4(&mWorldMatrix);
	...
	void Enemy::CreateVertexBuffer(ID3D11Device* device, const Mesh& mesh, ID3D11Buffer** vertexBuffer) const
	{
	...
	void Enemy::Patrol(const GameTime &gameTime)
	{
		float elapsedTime = (float)gameTime.ElapsedGameTime();
		
		if (pos.x > -50 && left == false && right == false)
		{
		...
		if (right == true)
		{
			SetPosition(0, 3.14, 0.0f, 1.0f, pos.x, 1, pos.z);
		}

		if (right == false)
		{
			SetPosition(0, 0, 0.0f, 1.0f, pos.x, 1, pos.z);
		}
